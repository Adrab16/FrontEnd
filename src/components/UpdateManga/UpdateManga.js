...
function UpdateManga() {
  ...
  useEffect(() =>{
    axios.get(`http://localhost:5000/ajouterunmanga/${params.id}`)
    .then((response) =>{
  ...
  <h1>{loading ? 'loading...' : manga.titre }</h1>
    ...
    {loading ? 'loading...' : manga.description }
        ...
        <form action={`http://localhost:5000/ajouterunmanga/edit/${params.id}?_method=PUT`} method="post">
    ...
    <form action={`http://localhost:5000/ajouterunmanga/delete/${params.id}?_method=DELETE`} method="post">
